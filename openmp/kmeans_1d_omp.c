...
static void update_step_1d(const double *X, double *C, const int *assign, int N, int K){
    ...
    #pragma omp parallel
    {
        // Acumuladores locais: cada thread tem sua própria cópia
        double *local_sum = (double*)calloc((size_t)K, sizeof(double));
        int    *local_cnt = (int*)calloc((size_t)K, sizeof(int));

        // Divide o laço for entre as threads. Cada thread escreve em sua cópia local.
        #pragma omp for
        for(int i=0; i<N; i++){
            int a = assign[i];
            local_cnt[a] += 1;
        ...
        // Junta os resultados locais no resultado global de forma segura.
        // A região crítica agora é muito rápida, pois executa apenas uma vez por thread.
        #pragma omp critical
        {
            for(int c=0; c<K; c++){
        ...
        // Libera a memória dos acumuladores locais de cada thread
        free(local_sum);
        free(local_cnt);
    }

    // Atualiza os centroides usando os resultados globais já consolidados
    for(int c=0; c<K; c++){
